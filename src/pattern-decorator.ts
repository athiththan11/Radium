...
import Pattern = require('./pattern');

class PatternDecorator {
	// All custom log level specified in the configuration.
	private _configPattern: Pattern[];
	// Indicates in which file which log level highlights which ranges.
	private _cache: Map<vscode.Uri, Map<Pattern, vscode.DecorationOptions[]>>;

	public constructor() {
		this._configPattern = [];
		this._cache = new Map<vscode.Uri, Map<Pattern, vscode.DecorationOptions[]>>();
	}

	public updateConfiguration(): void {
		const configPatterns = vscode.workspace.getConfiguration('radium').get('patterns') as {
			pattern: string;
			foreground?: string;
			background?: string;
			fontWeight?: string;
	...
	public decorateDocument(changedEvent: vscode.TextDocumentChangeEvent): void {
		...
		const contentToEnd: string = doc.getText(new vscode.Range(startPos, doc.lineAt(doc.lineCount - 1).range.end));
		...
		for (const logLevel of this._configPattern) {
			...
			const logLevelOptions = patternCache?.filter((option) => {
			...
			for (const regex of logLevel.regexes) {
				...
				while (matches) {
					...
					logLevelOptions?.push({
						hoverMessage: logLevel.tip,
			...
			docCache?.set(logLevel, logLevelOptions!);
			editors[0].setDecorations(logLevel.decoration, logLevelOptions!);
		}
	...
	public decorateEditors(editors: vscode.TextEditor[], changes?: vscode.TextDocumentContentChangeEvent[]): void {
		if (editors.length >= 1) {
			for (const editor of editors) {
				...
				const docRanges = new Map<Pattern, vscode.DecorationOptions[]>();
				...
				for (const logLevel of this._configPattern) {
					...
					// Update cache and set decorations.
					editor.setDecorations(logLevel.decoration, logLevelOptions);
					docRanges.set(logLevel, logLevelOptions);
				}
